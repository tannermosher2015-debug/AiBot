...
const dayna = {
  ...
  runTool(name, input) {
    if (name === "book_appointment") {
      ...
      return `Appointment confirmed and passed to the ${DAYNA.brokerage} team. A confirmation will be sent to ${input.contact}.`;
    }
...
const frontline = {
  ...
  runTool(name, input) {
    if (name === "book_call") {
      ...
      console.log(JSON.stringify(input, null, 2), "\n");
      return `Call confirmed — Frontline AI will reach out at ${input.contact} to confirm ${input.preferred_time}.`;
    }
    return `Unknown tool: ${name}`;
  },
};
