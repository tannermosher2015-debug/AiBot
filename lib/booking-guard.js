...
const BOOKING_CLAIM_PATTERNS = [
  // Up to two words may sit between "lock" and "in". Measured 2026-08-26: the bot
  // opened with "happy to lock that in" before anything was booked, and the original
  // pattern (/lock(ed)? in/) missed it on the single word "that".
  /\block(?:ed|ing)?\b(?:\s+\S+){0,2}\s+\bin\b/i,
  /\bpencil(?:ed)?\b(?:\s+\S+){0,2}\s+\bin\b/i,
  /\bgot you (?:down|booked|scheduled|in)\b/i,
  /\b(?:saved|holding|held|reserved) (?:you |your |that |the )?(?:spot|slot|time)\b/i,
  /\ball set\b/i,
...
// Em and en dashes are banned in Frontline copy, and the model writes them anyway:
// a live reply on 2026-09-19 used one. The repo's own strings were cleaned in
// 9232226; this covers the half a commit cannot reach, which is what the model
// generates at runtime. Same reasoning as the booking guard above: a prompt is a
// nudge, this is the guarantee.
//
// Only inline whitespace is consumed, never a newline, so a line that starts with
// a dash does not get pulled onto the previous line.
const DASHES = /[^\S\n]*[\u2014\u2013][^\S\n]*/g;

export function stripDashes(reply) {
  if (typeof reply !== "string") return reply;
  return reply.replace(DASHES, " - ");
}
