...
import { bookingLink } from "./leads.js";
...
// ── Model ──────────────────────────────────────────────────────────────
// Sonnet, chosen 2026-08-26 when this widget went from one page to all 46 of
// frontlinewebdesign.tech. Qualifying a lead is not an Opus-grade task and every
// visitor who opens the bubble spends tokens. Revisit only if reply quality drops.
...
export async function runChat(incomingMessages, agent = AGENT) {
  ...
  let bookingsUsed = 0;
  // Set only by a tool result that actually confirmed a reservation.
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    if (response.stop_reason === "tool_use") {
      ...
      for (const block of response.content) {
        ...
        if (typeof result === "string" && result.startsWith("CONFIRMED.")) bookingConfirmed = true;
    ...
    messages.push({ role: "assistant", content: response.content });
    return {
      reply: stripDashes(guardBookingClaims(reply, { booked: bookingConfirmed, bookingUrl: bookingLink() })),
      messages,
    };
  }
