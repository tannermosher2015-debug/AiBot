...
import { guardBookingClaims, stripDashes } from "./booking-guard.js";
...
check("replaces a spaced em dash with a spaced hyphen", () => {
  assert.equal(stripDashes("we build sites \u2014 fast"), "we build sites - fast");
});

check("replaces an unspaced em dash too", () => {
  assert.equal(stripDashes("open 9am\u20145pm"), "open 9am - 5pm");
});

check("replaces an en dash", () => {
  assert.equal(stripDashes("Mon\u2013Fri"), "Mon - Fri");
});

check("leaves a clean reply untouched", () => {
  const s = "We build sites - fast. Want a free consultation?";
  assert.equal(stripDashes(s), s);
});

check("does not pull a dash-led line onto the line above", () => {
  const out = stripDashes("What we do:\n\u2014 websites\n\u2014 AI chat");
  assert.equal(out.split("\n").length, 3, "newlines were eaten: " + JSON.stringify(out));
  assert.ok(out.indexOf("\u2014") === -1, "a dash survived");
});

check("survives junk input without throwing", () => {
  for (const bad of [null, undefined, 42, {}]) {
    assert.doesNotThrow(() => stripDashes(bad));
  }
});

// A strip that never fires would pass every "unchanged" test above.
console.log("\n  control: the dash strip must actually be capable of firing");
const before = "a \u2014 b";
const after = stripDashes(before);
if (after === before) {
  console.log("  BAD  stripDashes changed nothing - it is inert");
  process.exit(1);
}
console.log("  ok   it fires: " + JSON.stringify(before) + " -> " + JSON.stringify(after));
