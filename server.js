...
// lead bot, switch to "claude-sonnet-4-6" (cheaper, still excellent) or
// "claude-haiku-4-5" (cheapest + fastest). One-line change.
const MODEL = "claude-opus-4-8";

// ── Per-client config (this is what you'd customize for each agent) ──────
const AGENT = {
...
// Demo listing data (sample Maui/Molokai listings). In production this is
// replaced by a live feed from the agent's MLS/IDX via an approved vendor
// (SimplyRETS, Trestle, Bridge, Spark, or their IDX feed). See README.
const LISTINGS = JSON.parse(
  readFileSync(path.join(__dirname, "listings.json"), "utf8")
);
  ...
  try {
    ...
    while (true) {
      ...
      if (response.stop_reason === "tool_use") {
        // Preserve the assistant turn (it contains the tool_use block).
        ...
        continue; // loop so Claude can respond to the lead after using a tool
      }
...
app.listen(PORT, () => {
  ...
  console.log(`    Model: ${MODEL}  ·  Listings loaded: ${LISTINGS.length}\n`);
});
